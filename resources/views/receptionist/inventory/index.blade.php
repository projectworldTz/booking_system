...
@section('page-title', __('Inventory & Assets'))

@section('content')

{{-- ── Header ─────────────────────────────────────────────────────────────── --}}
<div class="mb-5 flex flex-wrap items-center justify-between gap-3">
    <h2 class="text-xl font-bold text-slate-900 dark:text-white">{{ __('Asset Register') }}</h2>
...
<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5 mb-6">
    @foreach([
        [__('Total Assets'),  $summary['total'],       'text-slate-700 dark:text-white',           'bg-slate-100 dark:bg-slate-700'],
        [__('Active'),        $summary['active'],      'text-emerald-600 dark:text-emerald-400',   'bg-emerald-50 dark:bg-emerald-900/20'],
        [__('Maintenance'),   $summary['maintenance'], 'text-amber-600 dark:text-amber-400',       'bg-amber-50 dark:bg-amber-900/20'],
        [__('Damaged'),       $summary['damaged'],     'text-rose-600 dark:text-rose-400',         'bg-rose-50 dark:bg-rose-900/20'],
        [__('Total Value'),   money($summary['total_value']), 'text-purple-600 dark:text-purple-400', 'bg-purple-50 dark:bg-purple-900/20'],
    ] as [$label, $val, $tc, $bg])
...
<div class="grid gap-6 lg:grid-cols-4">
    ...
    <div class="lg:col-span-1">
        <div class="rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm p-4">
            <h3 class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">{{ __('By Category') }}</h3>
            ...
            @endphp
            <a href="{{ route('receptionist.inventory.index', ['category' => $row->asset_category_id]) }}"
            ...
            @empty
            <p class="text-xs text-slate-400">{{ __('No assets yet.') }}</p>
            @endforelse

            @if(request()->hasAny(['category','status','condition','search']))
            <a href="{{ route('receptionist.inventory.index') }}" class="mt-3 block text-xs text-center text-slate-400 hover:text-slate-600 transition">{{ __('Clear filters') }}</a>
            @endif
        </div>
    </div>
    ...
    <div class="lg:col-span-3 space-y-4">
        ...
        <form method="GET" class="flex flex-wrap gap-2">
            ...
            <select name="condition" class="form-input w-auto text-sm">
                ...
                @foreach(['excellent'=>__('Excellent'),'good'=>__('Good'),'fair'=>__('Fair'),'poor'=>__('Poor'),'damaged'=>__('Damaged')] as $v => $l)
            ...
            <select name="status" class="form-input w-auto text-sm">
                ...
                <option value="under_maintenance" @selected(request('status') === 'under_maintenance')>{{ __('Maintenance') }}</option>
                    ...
                    <tbody>
                        ...
                        <tr class="tr-hover" x-data="{ editOpen: false }">
                            ...
                            <td>
                                ...
                                <p class="text-xs text-slate-400 truncate max-w-[160px]">{{ $asset->description }}</p>
                                @endif
                            </td>
                            ...
                            <td class="text-xs">
                                @if($asset->warranty_expires_at)
                                    @if($warrantyStatus === 'expired')
                                        <span class="text-rose-500">{{ __('Expired') }}</span>
                                    @elseif($warrantyStatus === 'expiring')
                                        <span class="text-amber-500">{{ $asset->warranty_expires_at->format('d M Y') }}</span>
                                    @else
                                ...
                                <div x-show="editOpen" x-trap="editOpen" @click.outside="editOpen = false"
                                    ...
                                    <div class="relative w-full max-w-lg rounded-2xl bg-white dark:bg-slate-800 shadow-2xl p-6 z-10 max-h-[90vh] overflow-y-auto">
                                        <div class="flex items-center justify-between mb-5">
                                            <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ __('Edit Asset') }}</h3>
                                            <button @click="editOpen = false" class="text-slate-400 hover:text-slate-600 transition">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                            </button>
                                        </div>
                                        @include('owner.inventory._form', [
                                            'asset'      => $asset,
                                            'categories' => $categories,
                                            'hotel'      => $hotel,
                                            'action'     => route('receptionist.inventory.update', $asset),
                                            'method'     => 'PUT',
                                        ])
                                    </div>
                                </div>
                        ...
                        <tr>
                            <td colspan="9" class="py-14 text-center">
                                ...
                                <p class="text-slate-500 font-medium">{{ __('No assets found.') }}</p>
                                <p class="text-slate-400 text-sm mt-1">{{ __('Add your first asset using the button above.') }}</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
...
<div x-data="{ open: false }"
    ...
    <div class="relative w-full max-w-lg rounded-2xl bg-white dark:bg-slate-800 shadow-2xl p-6 z-10 max-h-[90vh] overflow-y-auto" @click.stop>
        <div class="flex items-center justify-between mb-5">
            <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ __('Add New Asset') }}</h3>
        ...
        @include('owner.inventory._form', [
            ...
            'categories' => $categories,
            'hotel'      => $hotel,
            'action'     => route('receptionist.inventory.store'),
            'method'     => 'POST',
        ])
    </div>
</div>
...
<div x-data="{ open: false }"
    ...
    <div class="relative w-full max-w-md rounded-2xl bg-white dark:bg-slate-800 shadow-2xl p-6 z-10 max-h-[90vh] overflow-y-auto" @click.stop>
        <div class="flex items-center justify-between mb-5">
            <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ __('Manage Categories') }}</h3>
            <button @click="open = false" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>

        <form method="POST" action="{{ route('receptionist.inventory.categories.store') }}" class="flex gap-2 mb-5">
            @csrf
            <input type="text" name="name" required maxlength="100" placeholder="{{ __('e.g. Furniture') }}"
        ...
        <div class="space-y-1.5">
            ...
            <div class="flex items-center justify-between rounded-lg px-3 py-2 bg-slate-50 dark:bg-slate-700/50">
                ...
                <form method="POST" action="{{ route('receptionist.inventory.categories.destroy', $cat) }}"
                      onsubmit="return confirm('{{ __('Delete category') }} {{ addslashes($cat->name) }}?')">
                    @csrf @method('DELETE')
            ...
            @empty
            <p class="text-xs text-slate-400 text-center py-3">{{ __('No categories yet — add one above.') }}</p>
            @endforelse
        </div>
    </div>
</div>

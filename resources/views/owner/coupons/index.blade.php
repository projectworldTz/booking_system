...
@section('title', __('Coupons') . ' — ' . $hotel->name)
@section('page-title', $hotel->name . ' — ' . __('Coupons'))

@section('content')
<div class="mb-4 flex items-center gap-2">
    <a href="{{ route('owner.hotels.show', $hotel) }}" class="btn-ghost btn-sm">← {{ $hotel->name }}</a>
    <a href="{{ route('owner.hotels.coupons.create', $hotel) }}" class="btn-primary btn-sm ml-auto">+ {{ __('New Coupon') }}</a>
</div>
...
<div class="mb-5 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-4 flex gap-3">
    ...
    <div class="text-sm text-blue-700 dark:text-blue-300">
        <p class="font-semibold">{{ __('How to share coupon codes with guests') }}</p>
        <p class="mt-0.5">{{ __('Copy the code from this list and share it via WhatsApp, SMS, your social media pages, or print it on a card to give guests at check-out. Guests enter the code at checkout to get their discount.') }}</p>
    </div>
</div>
...
<div class="card">
    @if($coupons->isEmpty())
        <div class="p-8 text-center text-slate-400 dark:text-slate-500">
            ...
            <p class="text-xs mt-1">{{ __('Create a coupon and share the code to attract more bookings.') }}</p>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr class="tr-hover">
                    ...
                    <td class="text-sm">
                        {{ $coupon->roomType->name ?? __('All rooms') }}
                    </td>
                    <td class="text-sm">
                        {{ $coupon->uses }}
                        @if($coupon->max_uses)
                    ...
                    <td>
                        <form method="POST" action="{{ route('owner.hotels.coupons.destroy', [$hotel, $coupon]) }}"
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-xs text-rose-500 hover:text-rose-700 font-medium">{{ __('Delete') }}</button>
                        </form>
                    </td>
                </tr>
                @endforeach

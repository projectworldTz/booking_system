...
@section('title', __('New Coupon') . ' — ' . $hotel->name)
@section('page-title', __('Create Coupon'))

@section('content')
<div class="max-w-2xl">
    <div class="mb-4">
        <a href="{{ route('owner.hotels.coupons.index', $hotel) }}" class="btn-ghost btn-sm">← {{ __('Coupons') }}</a>
    </div>
    ...
    <form method="POST" action="{{ route('owner.hotels.coupons.store', $hotel) }}">
        ...
        <div class="space-y-5">
            ...
            <div class="card p-6 space-y-5">
                <h2 class="text-base font-bold text-slate-900 dark:text-white">{{ __('Coupon Details') }}</h2>

                {{-- Code --}}
                <div>
                    <label class="form-label">{{ __('Coupon Code') }}
                        <span class="text-slate-400 font-normal">({{ __('leave blank to auto-generate') }})</span>
                    </label>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <select name="type" class="form-select @error('type') border-rose-500 @enderror" required>
                            ...
                            <option value="fixed" {{ old('type') === 'fixed' ? 'selected' : '' }}>{{ __('Fixed Amount') }} (TZS)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="value" value="{{ old('value') }}"
                               ...
                               placeholder="{{ __('e.g. 10 for 10% or 10000 for TZS 10,000') }}" required>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-400">{{ __('Restrict to a specific room type, or apply to all rooms.') }}</p>
                </div>
                @endif
            </div>

            <div class="card p-6 space-y-5">
                <h2 class="text-base font-bold text-slate-900 dark:text-white">{{ __('Restrictions') }}</h2>

                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="form-label">{{ __('Minimum Booking Amount (TZS)') }}</label>
                        <input type="number" name="min_booking_amount" value="{{ old('min_booking_amount') }}"
                               min="0" step="1000" class="form-input"
                               placeholder="{{ __('No minimum if empty') }}">
                        @error('min_booking_amount') <p class="form-error">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="form-label">{{ __('Max Uses') }}</label>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-400">{{ __('Leave empty for a coupon that never expires.') }}</p>
                </div>
            </div>

            <div class="flex gap-3">
                <button type="submit" class="btn-primary">{{ __('Create Coupon') }}</button>
                <a href="{{ route('owner.hotels.coupons.index', $hotel) }}" class="btn-ghost">{{ __('Cancel') }}</a>
            </div>
        </div>
    </form>
</div>

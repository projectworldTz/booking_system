...
<div class="card table-wrap">
    <table class="table">
        <thead>
            <tr><th>{{ __('Hotel') }}</th><th>{{ __('City') }}</th><th>{{ __('Stars') }}</th><th>{{ __('Rooms') }}</th><th>{{ __('Status') }}</th><th></th></tr>
        </thead>
        <tbody>
            ...
            <tr class="tr-hover">
                ...
                <td>
                    <div class="flex gap-1">
                        <a href="{{ route('owner.hotels.show', $hotel) }}" class="btn-ghost btn-sm">{{ __('View') }}</a>
                        <a href="{{ route('owner.hotels.edit', $hotel) }}" class="btn-outline btn-sm">{{ __('Edit') }}</a>
                    </div>
                </td>
            </tr>
            @endforeach

            ...
            @else
                <div class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                    ...
                    <a href="{{ route('hotels.show', $hotel) }}"
                       class="card group hover:shadow-xl transition hover:-translate-y-0.5">
                        <div class="relative h-44 overflow-hidden bg-slate-200 dark:bg-slate-700">
                            ...
                            @endif
                            <div class="absolute top-2 left-2 flex rounded-full bg-white/90 px-1.5 py-0.5 shadow">
                        ...
                        <div class="p-4">
                            ...
                            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ $hotel->city }}, {{ $hotel->country }}</p>
                            <div class="mt-3 flex items-center justify-between">
                                @if($hotel->average_rating)
                                    <span class="text-xs font-medium text-emerald-700 dark:text-emerald-400">★ {{ number_format($hotel->average_rating, 1) }}</span>
                                @else
                                    <span></span>
                                @endif
                                <span class="text-sm font-bold text-navy dark:text-navy-light">
                                    @if($hotel->roomTypes->isNotEmpty())
                                        {{ __('from') }} TZS {{ number_format($hotel->roomTypes->min('base_price'), 0) }}/{{ __('night') }}
                                    @endif
                                </span>
                            </div>
                        </div>

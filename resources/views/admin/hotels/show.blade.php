...
@section('title', $hotel->name . ' — Hotel Management')
...
<div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-5">
    <a href="{{ route('admin.hotels.index') }}" class="hover:text-navy dark:hover:text-amber-400 transition">Hotels</a>
    <span>/</span>
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500">
                    ...
                    &middot; Owner: <span class="font-medium text-slate-700 dark:text-slate-300">{{ $hotel->owner->name ?? 'N/A' }}</span>
                    @if($hotel->is_featured)
                        &middot; <span class="text-amber-500 font-medium">★ Featured</span>
                    @endif
                </p>
            </div>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <form method="POST" action="{{ route('admin.hotels.suspend', $hotel) }}">
                @csrf
                <button class="btn-danger btn-sm" onclick="return confirm('Suspend this hotel?')">Suspend</button>
            </form>
            @endif
            ...
            <form method="POST" action="{{ route('admin.hotels.approve', $hotel) }}">
                @csrf
                <button class="btn-success btn-sm">Reactivate</button>
            </form>
            @endif
...
<div class="border-b border-slate-200 dark:border-slate-700 mb-6 -mx-1">
    <nav class="flex gap-1 overflow-x-auto px-1">
        @php
            $tabs = [
                ['id' => 'overview',  'label' => 'Overview',  'route' => route('admin.hotels.show', $hotel)],
                ['id' => 'bookings',  'label' => 'Bookings',  'route' => route('admin.hotels.hub.bookings', $hotel)],
                ['id' => 'revenue',   'label' => 'Revenue',   'route' => route('admin.hotels.hub.revenue', $hotel)],
                ['id' => 'rooms',     'label' => 'Rooms',     'route' => route('admin.hotels.hub.rooms', $hotel)],
                ['id' => 'staff',     'label' => 'Staff',     'route' => route('admin.hotels.hub.staff', $hotel)],
                ['id' => 'guests',    'label' => 'Guests',    'route' => route('admin.hotels.hub.guests', $hotel)],
                ['id' => 'features',  'label' => '★ Features', 'route' => route('admin.hotels.hub.features', $hotel)],
            ];
        @endphp
        @foreach($tabs as $t)
        <a href="{{ $t['route'] }}"
...
<div class="space-y-6">
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        @foreach([
            ['Total Bookings',  $stats['total']        ?? 0,  'text-slate-900 dark:text-white', 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
            ['Active',          ($stats['pending']??0) + ($stats['confirmed']??0) + ($stats['checked_in']??0), 'text-emerald-600 dark:text-emerald-400', 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
            ['This Month',      '$' . number_format($stats['revenue_month'] ?? 0, 0), 'text-navy dark:text-amber-400', 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
            ['Total Revenue',   '$' . number_format($stats['total_revenue'] ?? 0, 0), 'text-navy dark:text-amber-400', 'M9 14.25l6-6m4.5-3.493V21.75l-3.75-1.5-3.75 1.5-3.75-1.5-3.75 1.5V4.757c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0c1.1.128 1.907 1.077 1.907 2.185z'],
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2 card p-5">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-bold text-slate-900 dark:text-white">Revenue (Last 12 Months)</h3>
                <a href="{{ route('admin.hotels.hub.revenue', $hotel) }}" class="btn-ghost btn-sm">Full Report →</a>
            </div>
            <div class="h-52">
        ...
        <div class="card p-5 text-sm space-y-2.5">
            <h3 class="font-bold text-slate-900 dark:text-white mb-3">Hotel Details</h3>
            @foreach([
                ['Star Rating', ($hotel->star_rating ?? '—') . '★'],
                ['Category',    $hotel->category->name ?? '—'],
                ['Rooms',       ($stats['total_rooms'] ?? 0) . ' total'],
                ['Room Types',  $hotel->roomTypes->count()],
                ['Check-in',    $hotel->check_in_time  ?? '14:00'],
                ['Check-out',   $hotel->check_out_time ?? '11:00'],
                ['Phone',       $hotel->phone ?? '—'],
                ['Email',       $hotel->email ?? '—'],
                ['Listed',      $hotel->created_at->format('d M Y')],
            ] as [$k, $v])
    ...
    <div class="card">
        <div class="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-700">
            ...
            <a href="{{ route('admin.hotels.hub.bookings', $hotel) }}" class="btn-ghost btn-sm">View All →</a>
        </div>
        <div class="table-wrap">
            <table class="table">
                <thead><tr><th>Booking #</th><th>Guest</th><th>Dates</th><th>Amount</th><th>Status</th></tr></thead>
                <tbody>
                    ...
                    <tr class="tr-hover">
                        ...
                        <td class="font-semibold">${{ number_format($b->grand_total, 2) }}</td>
...
<form method="GET" class="flex flex-wrap gap-3 mb-5">
    ...
    <input type="date" name="date_from" value="{{ $filters['date_from'] ?? '' }}" class="form-input w-auto text-sm" placeholder="From">
    <input type="date" name="date_to"   value="{{ $filters['date_to']   ?? '' }}" class="form-input w-auto text-sm" placeholder="To">
    <input type="search" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Booking # or guest name…" class="form-input flex-1 min-w-48 text-sm">
    <button type="submit" class="btn-primary btn-sm">Filter</button>
    @if(array_filter($filters ?? []))
    <a href="{{ route('admin.hotels.hub.bookings', $hotel) }}" class="btn-ghost btn-sm">Clear</a>
    @endif
</form>
...
<div class="card">
    <div class="table-wrap">
        <table class="table">
            <thead>
                <tr><th>Booking #</th><th>Guest</th><th>Check-in</th><th>Check-out</th><th>Nights</th><th>Amount</th><th>Status</th><th></th></tr>
            </thead>
            <tbody>
                ...
                @empty
                <tr><td colspan="8" class="py-10 text-center text-slate-400">No bookings found for this hotel.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="space-y-6">
    ...
    <form method="GET" class="flex items-center gap-3">
        <label class="text-sm text-slate-600 dark:text-slate-400">Period:</label>
    ...
    <div class="grid gap-4 sm:grid-cols-3">
        @foreach([
            ['Total Revenue',  '$' . number_format($stats['total_revenue'] ?? 0, 0)],
    ...
    {{-- Revenue chart --}}
    <div class="card p-5">
        <h3 class="font-bold text-slate-900 dark:text-white mb-4">Monthly Revenue</h3>
    ...
    <div class="card">
        <div class="table-wrap">
            <table class="table">
                <thead><tr><th>Month</th><th class="text-right">Bookings</th><th class="text-right">Revenue</th></tr></thead>
                <tbody>
                    ...
                    @empty
                    <tr><td colspan="3" class="py-8 text-center text-slate-400">No revenue data.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
<div class="space-y-5">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500">
                        {{ $rt->beds_count }}× {{ $rt->bed_type }} · Max {{ $rt->max_guests }} guests · ${{ number_format($rt->base_price, 2) }}/night
                    </p>
                </div>
            </div>
            <span class="badge badge-confirmed">{{ $rt->rooms->count() }} rooms</span>
        </div>
        @if($rt->rooms->isNotEmpty())
        <div class="table-wrap">
            <table class="table">
                <thead><tr><th>Room #</th><th>Floor</th><th>Status</th><th>Notes</th></tr></thead>
                <tbody>
    ...
    @empty
    <div class="card p-10 text-center text-slate-400">No room types configured yet.</div>
    @endforelse
</div>
...
<div class="card">
    <div class="table-wrap">
        <table class="table">
            <thead><tr><th>Name</th><th>Email</th><th>Position</th><th>Status</th><th>Since</th></tr></thead>
            <tbody>
                ...
                @empty
                <tr><td colspan="5" class="py-10 text-center text-slate-400">No staff assigned to this hotel.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<form method="GET" class="flex gap-3 mb-5">
    <input type="search" name="search" value="{{ $search ?? '' }}"
           placeholder="Search guest name or email…" class="form-input flex-1 text-sm">
    <button type="submit" class="btn-primary btn-sm">Search</button>
    @if($search ?? false)
    <a href="{{ route('admin.hotels.hub.guests', $hotel) }}" class="btn-ghost btn-sm">Clear</a>
    @endif
</form>
...
<div class="card">
    <div class="table-wrap">
        <table class="table">
            <thead><tr><th>Guest</th><th>Email</th><th class="text-center">Bookings</th><th class="text-right">Total Spent</th><th>Last Visit</th></tr></thead>
            <tbody>
                @forelse($guests as $g)
                <tr class="tr-hover">
                    ...
                    <td class="text-right font-semibold text-emerald-600 dark:text-emerald-400">
                        ${{ number_format($g->hotel_spend ?? 0, 2) }}
                    </td>
                    <td class="text-xs text-slate-400">{{ $g->updated_at->diffForHumans() }}</td>
                </tr>
                @empty
                <tr><td colspan="5" class="py-10 text-center text-slate-400">No guests found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            ...
            <div class="relative rounded-2xl border {{ $active ? $tc['border'] . ' ' . $tc['bg'] : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800' }} p-5 transition-all">
                ...
                <div class="mb-3 rounded-xl bg-white/60 dark:bg-slate-900/40 border border-white/80 dark:border-slate-600 px-3 py-2 text-xs space-y-0.5">
                    ...
                    <div class="flex justify-between">
                        <span class="text-slate-500">Expires</span>
                        <span class="font-medium text-emerald-600 dark:text-emerald-400">Never (permanent)</span>
                    </div>
                    @endif
                    @if($granted->notes)
                ...
                @else
                    <details class="group">
                        ...
                        <form method="POST" action="{{ route('admin.hotels.features.grant', $hotel) }}"
                            ...
                            <input type="hidden" name="feature" value="{{ $feature->value }}">
                            <div>
                                <label class="block text-xs font-medium text-slate-500 mb-1">Expires (leave blank = permanent)</label>
                                <input type="date" name="expires_at"
                                       min="{{ now()->addDay()->toDateString() }}"
                                       class="form-input w-full text-xs">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-500 mb-1">Agreement notes (optional)</label>
                                <input type="text" name="notes" placeholder="e.g. 6-month trial, contract #1234"
                                       class="form-input w-full text-xs">
                            </div>
                            <button type="submit"

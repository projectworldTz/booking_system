@extends('layouts.app')
@section('title', __('Saved Hotels'))

@section('content')
<div class="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="page-header">
        <h1 class="page-title">{{ __('Saved Hotels') }}</h1>
    </div>
    ...
    @if($favorites->isEmpty())
        <div class="card flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ __('No saved hotels yet') }}</h3>
            <p class="mt-1 text-sm text-slate-500">{{ __('Tap the heart on any hotel to save it here.') }}</p>
            <a href="{{ route('hotels.index') }}" class="btn-primary mt-5">{{ __('Explore Hotels') }}</a>
        </div>
    @else
        ...
        <div class="card group hover:shadow-xl transition">
            <div class="relative h-44 overflow-hidden bg-slate-200 dark:bg-slate-700">
                ...
                <form method="POST" action="{{ route('favorites.destroy', $fav) }}"
                    ...
                    @csrf
                    @method('DELETE')
                    <button type="submit" title="{{ __('Remove from saved') }}"
